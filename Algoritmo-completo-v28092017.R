...
#vasco
source("landsat2.R")
dados <- read.csv("LT05_L1TP_215065_19880612_20170208_01_T1.csv",sep=";", stringsAsFactors=FALSE) # Data
...
MTL<-read.table(dados$MTL[1],skip=0,nrows=140,sep="=", quote = "''",as.is=TRUE) #MTL File, #Leitura do MTL,nrows=-1
...
if (n.sensor==8) MTL<-read.table(dados$MTL[1],skip=0,nrows=-1,sep="=", quote = "''",as.is=TRUE,fill=TRUE)#versão alterada 01/08/2017
...
# Resample images
beginCluster(clusters)
raster.elevation<-resample(raster.elevation,raster.elevation.aux,method="ngb")#Compactibilizar a imagem
proc.time()
...
phase2 <- function() {
	...
	HO<-Rn[]-G[] # Era raster - leio so o vetor. HO <- VETOR
	x<-TS[][(NDVI[]>0.15 &!is.na(NDVI[]))  & (NDVI[]<0.20 &!is.na(NDVI[])) & (alb[]<0.22)] # Retorna Vetor
	x<-x[x>273.16]
	TS.c.hot<-sort(x)[round(0.95*length(x))] # Retorna um valor
	HO.c.hot<-HO[][(NDVI[]>0.15 &!is.na(NDVI[]))  & (NDVI[]<0.20 &!is.na(NDVI[]) & (alb[]<0.22)) & TS[]==TS.c.hot] # Retorna um valor
	...
	x<-TS[][(NDVI[]<0 & !is.na(NDVI[]))  & !is.na(HO) & (alb[]<0.08)]
	x<-x[x>273.16]
	...
	#zom for all pixels
	zom<-exp(azom+bzom*NDVI[]) #rASTER  MUDEI PARA VETOR
	...
	while(Erro){
	  rah.hot.0<-value.pixel.rah[i] #VALOR
	  #Hot and cold pixels      
	  dt.hot<-H.hot*rah.hot.0/(rho*cp) #VALOR                  
	  b<-dt.hot/(value.pixels.ref["hot","TS"]-value.pixels.ref["cold","TS"])  #VALOR
	  a<- -b*(value.pixels.ref["cold","TS"]-273.15) #VALOR                          
	  #All pixels
	  H<-rho*cp*(a+b*(TS[]-273.15))/rah[]   # RASTER   - vetor                               
	  L<- -1*((rho*cp*ustar[]^3*TS[])/(k*g*H)) # RASTER - VETOR                       
	  y_0.1<-(1-16*0.1/L)^0.25 # RASTER - VETOR 
	  y_2<-(1-16*2/L)^0.25      # RASTER                                        
	  x200<-(1-16*200/L)^0.25    # RASTER                                       
	  psi_0.1<-2*log((1+y_0.1^2)/2)  # RASTER                                    
	...
	writeRaster(H1,filename="H",format="GTiff",overwrite=TRUE)
